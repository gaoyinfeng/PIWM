...
class Disag(nj.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    self.inputs = nets_original.Input(config.disag_head.inputs, dims='deter')
    self.target = nets_original.Input(self.config.disag_target, dims='deter')
    self.nets = [
        nets_original.MLP(shape=None, **self.config.disag_head, name=f'disag{i}')
