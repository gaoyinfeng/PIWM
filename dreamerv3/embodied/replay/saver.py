...
class Saver:
  ...
  def add(self, step, worker):
    if self.loading:
      return
    # only one worker for us for now
    buffer = self.buffers[worker] # buffer is Chunk
    buffer.append(step)
    # if buffer size is larger than one chunk, save it?
    if buffer.length >= self.chunks:
      # clear or initialize 'buffers[worker]', yet 'buffer' itself is unchanged
      self.buffers[worker] = buffer.successor = chunklib.Chunk(self.chunks)
      # for 'buffer' itself, save it
