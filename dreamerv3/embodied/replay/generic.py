...
class Generic:
  ...
  def add(self, step, worker=0, load=False):
    ...
    # if stream is smalled than length
    if len(stream) < self.length or self.counters[worker] < self.stride: # length is 64, stride is 1
      return
    
    # if worker collect data larger than length and stride, clear the timer
  ...
  def _sample(self):
    ...
    # make it into key:value format, value length
    seq = {k: [step[k] for step in seq] for k in seq[0]}
    # data type convert
    seq = {k: embodied.convert(v) for k, v in seq.items()}

    # force the 'is_first' of first step of each batch to be True
    if 'is_first' in seq:
      seq['is_first'][0] = True
    return seq
