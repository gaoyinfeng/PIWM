...
def scan(fn, inputs, start, unroll=True, modify=False):
  # seems like usally unroll is False
  # TODO: why * 2?
  fn2 = lambda carry, inp: (fn(carry, inp),) * 2
  # usally unroll is False
  if not unroll: 
...
class SymlogDist:
  ...
  def log_prob(self, value, valid=1): 
    # value is gt, for prediction task, valid is used in prediction decoder, 
    # it is a mask represents which gt is actually exist rather than zero-padding
    assert self._mode.shape == value.shape, (self._mode.shape, value.shape)
    # define how to calculate losses, config default is 'mse'
    if self._dist == 'mse':
      distance = (self._mode - symlog(value)) ** 2
      distance = jnp.where(valid, distance, 0)
    ...
    # define how to aggregate losses, config default is 'sum'
    if self._agg == 'mean':
      loss = distance.mean(self._dims)
    elif self._agg == 'sum':
      loss = distance.sum(self._dims)
...
class Optimizer(nj.Module):
  ...
  def __init__(
    ...
    # TODO: what is this?
    self.scaling = (COMPUTE_DTYPE == jnp.float16) # True
    if self.scaling:
