...
def grad(fun, keys, has_aux=False):
  ...
  def wrapper(*args, **kwargs):
    ...
    for mod in mods:
      strs += mod.getm()
    x1 = {k: v for k, v in context().items() if k in strs} # layers in module
    x2 = {k: v for k, v in context().items() if k not in strs} # layers not in module
...
class Module(object, metaclass=ModuleMeta):
  ...
  def get(self, name, *args, **kwargs):
    ...
    # if module dont exist, create it
    ctor, *args = args # for example, ctor is Initializer(...), *args is shape
