...
class JAXAgent(embodied.Agent):
  ...
  def predictor(self, obs, post_dict):
    # is only for testing i-sim env
    obs = obs.copy()
    obs = self._convert_inps(obs, self.policy_devices)
    rng = self._next_rngs(self.policy_devices)
    varibs = self.varibs if self.single_device else self.policy_varibs
    post_dict = tree_map(
        np.asarray, post_dict, is_leaf=lambda x: isinstance(x, list))
    post_dict = self._convert_inps(post_dict, self.policy_devices)

    prediction, _ = self._predictor(varibs, rng, obs, post_dict)
    prediction = self._convert_outs(prediction, self.policy_devices)
    return prediction
  ...
  def _transform(self):
    ...
    self._predictor = nj.pure(self.agent.predictor) # only for testing i-sim env
    self._train = nj.pure(self.agent.train)
    self._report = nj.pure(self.agent.report)
    # jit: compiles a pure function for fast execution; pnmp: compiles multiple pure functions for fast execution
    if len(self.train_devices) == 1:
    ...
    if len(self.policy_devices) == 1:
      ...
      self._predictor = nj.jit(self._predictor, **kw) # only for testing i-sim env, only decode vehicle's future prediction
    else:
      ...
      self._predictor = nj.pmap(self._predictor, 'i', **kw) # only for testing i-sim env, only decode vehicle's future prediction
